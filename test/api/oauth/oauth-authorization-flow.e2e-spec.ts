...
describe("OAuth2 Authorization Code Flow (e2e)", () => {
  ...
  describe("Complete Authorization Code Flow", () => {
    ...
    it("should complete full OAuth2 Authorization Code flow with PKCE for new user", async () => {
      ...
      const newUserEmail = EmailValue.fromString(
        `${IdentityValue.create().toString()}@example.com`,
      );
    ...
    it("should complete full OAuth2 Authorization Code flow with PKCE - with no existing user & no intent specified", async () => {
      const clientId = IdentityValue.create();
      const scope = defaultTestClientScopesMother();
      const redirectUri = RedirectUriValue.create(
        "https://client.com/callback",
      ...
      // Step 3: Choose to sign up (new user)
      const chooseSignUpResponse = await request(
        context.getApp().getHttpServer(),
      )
        .get("/oauth/prompt")
        .query({
      ...
      OauthE2eTestContext.expectIsRegistrationForm(chooseSignUpResponse);
      const csrfToken =
        OauthE2eTestContext.extractCSRFToken(chooseSignUpResponse);
      ...
      // Step 5: Continue with authorization flow after registration
      const getAuthorizationPromptResponse = await request(
        context.getApp().getHttpServer(),
      )
        .get("/oauth/prompt")
      ...
      // Step 6: Submit Authorization for newly registered user
      const postAuthorizationPromptResponse = await request(
        context.getApp().getHttpServer(),
      )
        .post("/oauth/prompt")
